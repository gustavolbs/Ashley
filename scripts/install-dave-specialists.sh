...
trap 'rm -rf "$TMP"' EXIT
echo "Installing optional Dave specialists from pinned Agency Agents ref $AGENCY_AGENTS_REF..."
...
(cd "$TMP/repo"; bash scripts/convert.sh --tool codex >/dev/null; bash scripts/install.sh --tool codex --agent "$(IFS=,; echo "${DAVE_SPECIALISTS[*]}")" --no-interactive --no-convert)
printf '\nInstalled Dave specialist roster:\n'; printf '  - %s\n' "${DAVE_SPECIALISTS[@]}"
