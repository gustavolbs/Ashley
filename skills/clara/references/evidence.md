...
## Clara-specific financial rule

A formula, spreadsheet or code path is not financially verified until the relevant financial invariants reconcile.

Depending on the flow, verify:
...
- source records vs derived totals.

Financial semantics and arithmetic are Clara's domain; implementation/runtime proof belongs with Dave and, where production payment infrastructure is involved, Guto.
