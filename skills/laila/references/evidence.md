...
Laila may only mark a work package complete when the owning persona returns evidence matching the package acceptance criteria. Status from a worker is input, not proof.

For cross-functional completion, distinguish domain evidence:
- business decision accepted;
- financial model/invariants reconciled;
- design artifact inspected;
- application behavior verified;
- operational rollout/health verified;
- marketing measurement validated.

If one required domain remains UNVERIFIED, the initiative is not fully verified.
